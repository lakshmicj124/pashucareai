...
DATASET_DIR = Path(r"c:\Users\DELL\Documents\cow disease project\dataset\goat")
MODEL_DIR = Path(r"c:\Users\DELL\Documents\cow disease project\ai-model")
MODEL_PATH = MODEL_DIR / "goat_model.pkl"

CLASSES_MAP = {
    "healthy": "Healthy",
    "mange": "Mange Dermatitis",
    "ppr": "Peste des Petits Ruminants (PPR)",
    "goat_pox": "Goat Pox",
    "orf": "Contagious Ecthyma (Orf)"
}
...
def load_and_split_dataset():
    ...
    for class_label, files in class_paths.items():
        if len(files) == 0:
            ...
            train_f, val_f = train_test_split(files, test_size=0.20, random_state=42)
    ...
    X_train = []
    y_train = []
    target_samples = 100  # Target number of samples per class for training
...
def train_model():
    ...
    if len(X_val) > 0:
        ...
        print(confusion_matrix(y_val, val_preds, labels=unique_classes))
        
    os.makedirs(MODEL_DIR, exist_ok=True)
    joblib.dump(clf, MODEL_PATH)
    print(f"\n[SUCCESS] Best goat model saved to: {MODEL_PATH}")
